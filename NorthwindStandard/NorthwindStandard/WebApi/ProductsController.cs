...
{
public partial class ProductsNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "ProductsNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.Product, dtoNW.Product> _factory 
	= new GenericFactory<entNW.Product, dtoNW.Product>();

		public ProductsNWController() : base()
		{
		}

		public ProductsNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Products/{productID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int productId)
		{
			try
			{
				...
				var result = Repo.DeleteProduct(productID);
		...
		[HttpGet]
		[VersionedRoute(template: "Products", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableProduct();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.Product).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		public async Task<IHttpActionResult> Get(int productId)
		{
			try
			{
				...
				var qryItem = Repo.GetQueryableProduct();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, productID);

				var dbItem = qryItem.Where(x => x.ProductId == productId).FirstOrDefault();
		...
		[HttpPatch]
		[VersionedRoute(template: "Products/{productID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int productId, [FromBody]JsonPatchDocument<dtoNW.Product> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetProduct(productID);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Products", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.Product dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.ProductID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Products/{productID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int productId, [FromBody]dtoNW.Product dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.Product newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.Product> dbItems, int productId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.Product> dbItems, ref List<string> filterList);
