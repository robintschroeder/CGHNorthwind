...
{
public partial class EmployeeTerritoriesNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "EmployeeTerritoriesNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.EmployeeTerritory, dtoNW.EmployeeTerritory> _factory 
	= new GenericFactory<entNW.EmployeeTerritory, dtoNW.EmployeeTerritory>();

		public EmployeeTerritoriesNWController() : base()
		{
		}

		public EmployeeTerritoriesNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "EmployeeTerritories/{employeeID}/{territoryID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int employeeId, string territoryId)
		{
			try
			{
				base.OnActionExecuting();

				var result = Repo.DeleteEmployeeTerritory(employeeID, territoryID);
		...
		[HttpGet]
		[VersionedRoute(template: "EmployeeTerritories", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableEmployeeTerritory();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.EmployeeTerritory).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "EmployeeTerritories/{employeeID}/{territoryID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int employeeId, string territoryId)
		{
			try
			{
				base.OnActionExecuting();

				var qryItem = Repo.GetQueryableEmployeeTerritory();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, employeeID, territoryID);
		...
		[HttpPatch]
		[VersionedRoute(template: "EmployeeTerritories/{employeeID}/{territoryID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int employeeId, string territoryId, [FromBody]JsonPatchDocument<dtoNW.EmployeeTerritory> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetEmployeeTerritory(employeeID, territoryID);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.TerritoryID = territoryID;
		...
		[HttpPost]
		[VersionedRoute(template: "EmployeeTerritories", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.EmployeeTerritory dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.EmployeeID}/{newDTOItem.TerritoryID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "EmployeeTerritories/{employeeID}/{territoryID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int employeeId, string territoryId, [FromBody]dtoNW.EmployeeTerritory dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.EmployeeTerritory newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.EmployeeTerritory> dbItems, int employeeId, string territoryId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.EmployeeTerritory> dbItems, ref List<string> filterList);
