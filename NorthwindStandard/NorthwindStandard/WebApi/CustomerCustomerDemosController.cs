...
{
public partial class CustomerCustomerDemosNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "CustomerCustomerDemosNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.CustomerCustomerDemo, dtoNW.CustomerCustomerDemo> _factory 
	= new GenericFactory<entNW.CustomerCustomerDemo, dtoNW.CustomerCustomerDemo>();

		public CustomerCustomerDemosNWController() : base()
		{
		}

		public CustomerCustomerDemosNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "CustomerCustomerDemos/{customerID}/{customerTypeID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(string customerId, string customerTypeId)
		{
			try
			{
				base.OnActionExecuting();

				var result = Repo.DeleteCustomerCustomerDemo(customerID, customerTypeID);
		...
		[HttpGet]
		[VersionedRoute(template: "CustomerCustomerDemos", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableCustomerCustomerDemo();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.CustomerCustomerDemo).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "CustomerCustomerDemos/{customerID}/{customerTypeID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(string customerId, string customerTypeId)
		{
			try
			{
				base.OnActionExecuting();

				var qryItem = Repo.GetQueryableCustomerCustomerDemo();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, customerID, customerTypeID);
		...
		[HttpPatch]
		[VersionedRoute(template: "CustomerCustomerDemos/{customerID}/{customerTypeID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(string customerId, string customerTypeId, [FromBody]JsonPatchDocument<dtoNW.CustomerCustomerDemo> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetCustomerCustomerDemo(customerID, customerTypeID);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.CustomerID = customerID;
				dtoItem.CustomerTypeID = customerTypeID;
		...
		[HttpPost]
		[VersionedRoute(template: "CustomerCustomerDemos", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.CustomerCustomerDemo dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.CustomerID}/{newDTOItem.CustomerTypeID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "CustomerCustomerDemos/{customerID}/{customerTypeID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(string customerId, string customerTypeId, [FromBody]dtoNW.CustomerCustomerDemo dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.CustomerCustomerDemo newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.CustomerCustomerDemo> dbItems, string customerId, string customerTypeId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.CustomerCustomerDemo> dbItems, ref List<string> filterList);
