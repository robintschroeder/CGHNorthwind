...
{
public partial class SuppliersNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "SuppliersNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.Supplier, dtoNW.Supplier> _factory 
	= new GenericFactory<entNW.Supplier, dtoNW.Supplier>();

		public SuppliersNWController() : base()
		{
		}

		public SuppliersNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Suppliers/{supplierID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int supplierId)
		{
			try
			{
				...
				var result = Repo.DeleteSupplier(supplierID);
		...
		[HttpGet]
		[VersionedRoute(template: "Suppliers", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableSupplier();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.Supplier).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		public async Task<IHttpActionResult> Get(int supplierId)
		{
			try
			{
				...
				var qryItem = Repo.GetQueryableSupplier();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, supplierID);

				var dbItem = qryItem.Where(x => x.SupplierId == supplierId).FirstOrDefault();
		...
		[HttpPatch]
		[VersionedRoute(template: "Suppliers/{supplierID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int supplierId, [FromBody]JsonPatchDocument<dtoNW.Supplier> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetSupplier(supplierID);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Suppliers", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.Supplier dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.SupplierID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Suppliers/{supplierID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int supplierId, [FromBody]dtoNW.Supplier dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.Supplier newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.Supplier> dbItems, int supplierId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.Supplier> dbItems, ref List<string> filterList);
