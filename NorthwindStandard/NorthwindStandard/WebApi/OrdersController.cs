...
{
public partial class OrdersNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "OrdersNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.Order, dtoNW.Order> _factory 
	= new GenericFactory<entNW.Order, dtoNW.Order>();

		public OrdersNWController() : base()
		{
		}

		public OrdersNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Orders/{orderID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int orderId)
		{
			try
			{
				...
				var result = Repo.DeleteOrder(orderID);
		...
		[HttpGet]
		[VersionedRoute(template: "Orders", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableOrder();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.Order).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		public async Task<IHttpActionResult> Get(int orderId)
		{
			try
			{
				...
				var qryItem = Repo.GetQueryableOrder();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, orderID);

				var dbItem = qryItem.Where(x => x.OrderId == orderId).FirstOrDefault();
		...
		[HttpPatch]
		[VersionedRoute(template: "Orders/{orderID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int orderId, [FromBody]JsonPatchDocument<dtoNW.Order> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetOrder(orderID);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Orders", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.Order dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.OrderID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Orders/{orderID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int orderId, [FromBody]dtoNW.Order dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.Order newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.Order> dbItems, int orderId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.Order> dbItems, ref List<string> filterList);
