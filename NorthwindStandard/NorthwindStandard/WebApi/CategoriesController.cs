...
{
public partial class CategoriesNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "CategoriesNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.Category, dtoNW.Category> _factory 
	= new GenericFactory<entNW.Category, dtoNW.Category>();

		public CategoriesNWController() : base()
		{
		}

		public CategoriesNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Categories/{categoryID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int categoryId)
		{
			try
			{
				...
				var result = Repo.DeleteCategory(categoryID);
		...
		[HttpGet]
		[VersionedRoute(template: "Categories", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableCategory();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.Category).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		public async Task<IHttpActionResult> Get(int categoryId)
		{
			try
			{
				...
				var qryItem = Repo.GetQueryableCategory();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, categoryID);

				var dbItem = qryItem.Where(x => x.CategoryId == categoryId).FirstOrDefault();
		...
		[HttpPatch]
		[VersionedRoute(template: "Categories/{categoryID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int categoryId, [FromBody]JsonPatchDocument<dtoNW.Category> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetCategory(categoryID);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Categories", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.Category dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.CategoryID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Categories/{categoryID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int categoryId, [FromBody]dtoNW.Category dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.Category newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.Category> dbItems, int categoryId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.Category> dbItems, ref List<string> filterList);
