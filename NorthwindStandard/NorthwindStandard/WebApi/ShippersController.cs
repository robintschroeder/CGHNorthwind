...
{
public partial class ShippersNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "ShippersNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.Shipper, dtoNW.Shipper> _factory 
	= new GenericFactory<entNW.Shipper, dtoNW.Shipper>();

		public ShippersNWController() : base()
		{
		}

		public ShippersNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Shippers/{shipperID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int shipperId)
		{
			try
			{
				base.OnActionExecuting();

				var result = Repo.DeleteShipper(shipperID);
		...
		[HttpGet]
		[VersionedRoute(template: "Shippers", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableShipper();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.Shipper).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Shippers/{shipperID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int shipperId)
		{
			try
			{
				base.OnActionExecuting();

				var qryItem = Repo.GetQueryableShipper();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, shipperID);

				var dbItem = qryItem.Where(x => x.ShipperId == shipperId).FirstOrDefault();
		...
		[HttpPatch]
		[VersionedRoute(template: "Shippers/{shipperID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int shipperId, [FromBody]JsonPatchDocument<dtoNW.Shipper> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetShipper(shipperID);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Shippers", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.Shipper dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.ShipperID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Shippers/{shipperID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int shipperId, [FromBody]dtoNW.Shipper dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.Shipper newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.Shipper> dbItems, int shipperId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.Shipper> dbItems, ref List<string> filterList);
