...
{
public partial class Order DetailsNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "Order DetailsNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.OrderDetail, dtoNW.OrderDetail> _factory 
	= new GenericFactory<entNW.OrderDetail, dtoNW.OrderDetail>();

		public Order DetailsNWController() : base()
		{
		}

		public Order DetailsNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Order Details/{orderID}/{productID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int orderId, int productId)
		{
			try
			{
				base.OnActionExecuting();

				var result = Repo.DeleteOrderDetail(orderID, productID);
		...
		[HttpGet]
		[VersionedRoute(template: "Order Details", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableOrderDetail();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.OrderDetail).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		public async Task<IHttpActionResult> Get(int orderId, int productId)
		{
			try
			{
				...
				var qryItem = Repo.GetQueryableOrderDetail();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, orderID, productID);
		...
		[HttpPatch]
		[VersionedRoute(template: "Order Details/{orderID}/{productID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int orderId, int productId, [FromBody]JsonPatchDocument<dtoNW.OrderDetail> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetOrderDetail(orderID, productID);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.OrderID = orderID;
				dtoItem.ProductID = productID;
		...
		[HttpPost]
		[VersionedRoute(template: "Order Details", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.OrderDetail dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.OrderID}/{newDTOItem.ProductID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Order Details/{orderID}/{productID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int orderId, int productId, [FromBody]dtoNW.OrderDetail dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.OrderDetail newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.OrderDetail> dbItems, int orderId, int productId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.OrderDetail> dbItems, ref List<string> filterList);
