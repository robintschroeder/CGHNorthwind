...
{
public partial class CustomerDemographicsNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "CustomerDemographicsNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.CustomerDemographic, dtoNW.CustomerDemographic> _factory 
	= new GenericFactory<entNW.CustomerDemographic, dtoNW.CustomerDemographic>();

		public CustomerDemographicsNWController() : base()
		{
		}

		public CustomerDemographicsNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "CustomerDemographics/{customerTypeID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(string customerTypeId)
		{
			try
			{
				base.OnActionExecuting();

				var result = Repo.DeleteCustomerDemographic(customerTypeID);
		...
		[HttpGet]
		[VersionedRoute(template: "CustomerDemographics", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableCustomerDemographic();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.CustomerDemographic).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "CustomerDemographics/{customerTypeID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(string customerTypeId)
		{
			try
			{
				base.OnActionExecuting();

				var qryItem = Repo.GetQueryableCustomerDemographic();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, customerTypeID);

				var dbItem = qryItem.Where(x => x.CustomerTypeId == customerTypeId).FirstOrDefault();
		...
		[HttpPatch]
		[VersionedRoute(template: "CustomerDemographics/{customerTypeID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(string customerTypeId, [FromBody]JsonPatchDocument<dtoNW.CustomerDemographic> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetCustomerDemographic(customerTypeID);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "CustomerDemographics", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.CustomerDemographic dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.CustomerTypeID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "CustomerDemographics/{customerTypeID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(string customerTypeId, [FromBody]dtoNW.CustomerDemographic dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.CustomerDemographic newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.CustomerDemographic> dbItems, string customerTypeId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.CustomerDemographic> dbItems, ref List<string> filterList);
