...
{
public partial class RegionsNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "RegionsNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.Region, dtoNW.Region> _factory 
	= new GenericFactory<entNW.Region, dtoNW.Region>();

		public RegionsNWController() : base()
		{
		}

		public RegionsNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Regions/{regionID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int regionId)
		{
			try
			{
				base.OnActionExecuting();

				var result = Repo.DeleteRegion(regionID);
		...
		[HttpGet]
		[VersionedRoute(template: "Regions", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableRegion();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.Region).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		public async Task<IHttpActionResult> Get(int regionId)
		{
			try
			{
				...
				var qryItem = Repo.GetQueryableRegion();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, regionID);

				var dbItem = qryItem.Where(x => x.RegionId == regionId).FirstOrDefault();
		...
		[HttpPatch]
		[VersionedRoute(template: "Regions/{regionID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int regionId, [FromBody]JsonPatchDocument<dtoNW.Region> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetRegion(regionID);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Regions", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.Region dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.RegionID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Regions/{regionID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int regionId, [FromBody]dtoNW.Region dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.Region newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.Region> dbItems, int regionId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.Region> dbItems, ref List<string> filterList);
