...
{
public partial class CustomersNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "CustomersNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.Customer, dtoNW.Customer> _factory 
	= new GenericFactory<entNW.Customer, dtoNW.Customer>();

		public CustomersNWController() : base()
		{
		}

		public CustomersNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Customers/{customerID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(string customerId)
		{
			try
			{
				...
				var result = Repo.DeleteCustomer(customerID);
		...
		[HttpGet]
		[VersionedRoute(template: "Customers", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableCustomer();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.Customer).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		public async Task<IHttpActionResult> Get(string customerId)
		{
			try
			{
				...
				var qryItem = Repo.GetQueryableCustomer();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, customerID);

				var dbItem = qryItem.Where(x => x.CustomerId == customerId).FirstOrDefault();
		...
		[HttpPatch]
		[VersionedRoute(template: "Customers/{customerID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(string customerId, [FromBody]JsonPatchDocument<dtoNW.Customer> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetCustomer(customerID);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Customers", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.Customer dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.CustomerID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Customers/{customerID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(string customerId, [FromBody]dtoNW.Customer dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.Customer newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.Customer> dbItems, string customerId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.Customer> dbItems, ref List<string> filterList);
