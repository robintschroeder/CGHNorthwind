...
{
public partial class TerritoriesNWController : NWBaseApiController
{
	private const string GET_LIST_ROUTE_NAME = "TerritoriesNWList";
	private const int maxPageSize = 100;

	private GenericFactory<entNW.Territory, dtoNW.Territory> _factory 
	= new GenericFactory<entNW.Territory, dtoNW.Territory>();

		public TerritoriesNWController() : base()
		{
		}

		public TerritoriesNWController(ILoggingService log, INWRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Territories/{territoryID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(string territoryId)
		{
			try
			{
				base.OnActionExecuting();

				var result = Repo.DeleteTerritory(territoryID);
		...
		[HttpGet]
		[VersionedRoute(template: "Territories", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableTerritory();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entNW.Territory).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Territories/{territoryID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(string territoryId)
		{
			try
			{
				base.OnActionExecuting();

				var qryItem = Repo.GetQueryableTerritory();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, territoryID);

				var dbItem = qryItem.Where(x => x.TerritoryId == territoryId).FirstOrDefault();
		...
		[HttpPatch]
		[VersionedRoute(template: "Territories/{territoryID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(string territoryId, [FromBody]JsonPatchDocument<dtoNW.Territory> patchDocument)
		{
			try
			{
				base.OnActionExecuting();
				...
				var dbItem = Repo.GetTerritory(territoryID);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Territories", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody]dtoNW.Territory dtoItem)
		{
			try
			{
				base.OnActionExecuting();
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.TerritoryID}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Territories/{territoryID}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(string territoryId, [FromBody]dtoNW.Territory dtoItem)
		{
			try
			{
				base.OnActionExecuting();
		...
		partial void RunCustomLogicAfterInsert(entNW.Territory newDBItem);

	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entNW.Territory> dbItems, string territoryId);
	...
	partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entNW.Territory> dbItems, ref List<string> filterList);
